...
RUN cat > /workspace/shared/package.template.json << 'EOF'
{
...
RUN cat > /workspace/shared/tsconfig.template.json << 'EOF'
{
  "compilerOptions": {
    "target": "ES2022",
...
RUN cat > /workspace/shared/vite.config.template.ts << 'EOF'
...
RUN cat > /workspace/init-agent.sh << 'EOF'
#!/bin/bash
set -e
