...
class ExtensionProxy {
  ...
  private async loadExtensions(): Promise<void> {
    try {
      ...
      for (const extension of extensions) {
        ...
        try {
          loadedExtension.compiledHandler = this.compileExtensionCode(
            extension.code,
          );
        } catch (error) {
          console.error(
            `[EXTENSION ERROR] Failed to compile ${extension.name}:`,
            error,
          );
          continue;
        }
  ...
  private compileExtensionCode(code: string): Function {
    ...
    const safeContext = {
      ...
      setTimeout: (fn: Function, delay: number) =>
        setTimeout(fn, Math.min(delay, 5000)), // Max 5s timeout
  ...
  private async getExtensionsForRoute(
    method: string,
    path: string,
  ...
  private async runBeforeExtensions(
    request: Request,
    ...
    for (const extension of this.extensions) {
      ...
      for (const routeHandler of extension.routes) {
        ...
        if (routeHandler.method !== route.method && routeHandler.method !== "*")
          continue;

        const pathMatches =
          typeof routeHandler.path === "string"
            ? route.path === routeHandler.path
            : routeHandler.path.test(route.path);

        if (pathMatches) {
          console.log(
            `[EXTENSION] Running ${extension.name} (before) on ${route.method} ${route.path}`,
          );
          try {
  ...
  private async runAfterExtensions(
    request: Request,
    response: Response,
    route: { method: string; path: string },
  ): Promise<Response> {
    ...
    for (const extension of this.extensions) {
      ...
      for (const routeHandler of extension.routes) {
        ...
        if (pathMatches) {
          console.log(
            `[EXTENSION] Running ${extension.name} (after) on ${route.method} ${route.path}`,
          );
          try {
            const result = await routeHandler.handler(
              request,
              modifiedResponse,
            );
  ...
  private async runReplaceExtensions(
    request: Request,
    route: { method: string; path: string },
    ...
    for (const extension of this.extensions) {
      ...
      for (const routeHandler of extension.routes) {
        ...
        if (pathMatches) {
          console.log(
            `[EXTENSION] Running ${extension.name} (replace) on ${route.method} ${route.path}`,
          );
          try {
  ...
  async handleRequest(request: Request): Promise<Response> {
    ...
    const replacementResponse = (await this.runExtensions(
      request,
      undefined,
      route,
      "replace",
